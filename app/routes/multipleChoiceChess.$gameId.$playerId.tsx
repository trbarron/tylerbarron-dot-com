...
export default function MultipleChoiceChessGame() {
  ...
  const showInactivityBanner =
    opponentInactiveMs !== null && opponentInactiveMs >= INACTIVITY_BANNER_MS;
  const canClaimWin =
    opponentInactiveMs !== null && opponentInactiveMs >= CLAIM_THRESHOLD_MS;
    ...
    <div className="flex min-h-screen flex-col">
        ...
        <section className="article">
          <article className="article-card">
            <header className="article-header">
              <h1 className="article-title">Multiple Choice Chess</h1>
            </header>
            <div className="prose">
              {error && (
                <div className="font-neo mb-4 flex items-center justify-between gap-3 border-4 border-red-500 bg-red-100 p-4 text-red-800">
                  <span>{error}</span>
                  {!engineReady &&
                    error.startsWith("Engine failed to load") && (
                      <button
                        onClick={retryEngineInit}
                        className="shrink-0 border-2 border-red-700 bg-white px-3 py-1 text-sm font-bold text-red-800 uppercase hover:bg-red-700 hover:text-white"
                      >
                        Retry
                      </button>
                    )}
              ...
              {showInactivityBanner && opponentInactiveMs !== null && (
                <div className="font-neo mb-4 flex flex-wrap items-center justify-between gap-3 border-4 border-amber-500 bg-amber-50 p-4 text-amber-900">
                  <div>
                    <div className="font-bold uppercase">Opponent inactive</div>
                    <div className="text-sm">
                      No response for {Math.floor(opponentInactiveMs / 1000)}s.
                      {!canClaimWin &&
                        ` You can claim a win after ${Math.ceil((CLAIM_THRESHOLD_MS - opponentInactiveMs) / 1000)}s.`}
                    </div>
                  </div>
                  {canClaimWin && (
                    <button
                      onClick={claimWin}
                      className="border-2 border-amber-700 bg-white px-3 py-1 text-sm font-bold text-amber-900 uppercase hover:bg-amber-700 hover:text-white"
                    >
                      Claim win
                    </button>
                  )}
              ...
              {phase === "waiting_opponent" && gameState && (
                <div className="font-neo mb-4 border-4 border-black bg-white p-6">
                  ...
                  <div className="mt-2 flex gap-2">
                    <input
                      ...
                      className="flex-1 border-2 border-black bg-gray-50 px-3 py-2 font-mono text-sm"
                    />
                    <button
                      onClick={copyLink}
                      className="font-neo border-2 border-black px-3 py-1 text-sm font-bold uppercase hover:bg-black hover:text-white active:bg-black active:text-white"
                    >
                  ...
                  {copyFailed && (
                    <p className="mt-2 text-xs text-red-700">
                      Couldn't access the clipboard. Press ⌘C / Ctrl+C to copy
                      the selected link.
                    </p>
                  )}
              ...
              <div className="grid gap-6 pb-6 md:grid-cols-5">
                <div className="md:col-span-3">
                  <Suspense
                    fallback={
                      <div className="font-neo flex aspect-square w-full items-center justify-center bg-gray-100 text-black">
                        Loading board...
                      </div>
                    }
                  >
                  ...
                  {viewingMoveIndex !== null && (
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                      <div className="font-neo border-2 border-black px-3 py-1 text-sm font-bold text-gray-600 uppercase">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() =>
                            setViewingMoveIndex(
                              Math.max(0, viewingMoveIndex - 1),
                            )
                          }
                          disabled={viewingMoveIndex === 0}
                          className="font-neo border-2 border-black px-3 py-1 text-sm font-bold uppercase hover:bg-black hover:text-white active:bg-black active:text-white disabled:opacity-40"
                        >
                          ←
                        </button>
                        <button
                  ...
                  {gameState?.status === "active" &&
                    ...
                    viewingMoveIndex === null && (
                      <div className="mt-3 flex items-center justify-between">
                        <div
                          className={[
                            "font-neo border-2 border-black px-3 py-1 text-sm font-bold uppercase",
                        ...
                        <button
                          ...
                          className="font-neo border-2 border-black px-3 py-1 text-sm font-bold text-gray-600 uppercase hover:border-red-500 hover:bg-red-100 hover:text-red-700 active:bg-red-100 active:text-red-700"
                        >
                          Resign
                        </button>
                      </div>
                ...
                <div className="space-y-4 md:col-span-2">
                  ...
                  {(phase === "opp_turn" || phase === "waiting_opponent") &&
                    gameState?.status === "active" && (
                      <EngineStatus
                        phase="opponent"
                        since={gameState.last_updated}
                      />
                    )}

                  {(phase === "my_choosing" || phase === "submitting") &&
                    candidates.length > 0 &&
                    viewingMoveIndex === null && (
                      <div>
                        <MoveChoices
      ...
      {confirmingResign && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="font-neo w-full max-w-sm border-4 border-black bg-white">
            ...
            <div className="grid grid-cols-2 divide-x-4 divide-black">
              <button
                ...
                className="px-4 py-3 text-sm font-bold uppercase hover:bg-black hover:text-white active:bg-black active:text-white"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-3 font-extrabold tracking-wide text-red-700 uppercase hover:bg-red-600 hover:text-white active:bg-red-600 active:text-white"
              >
                Resign
              </button>
            </div>
