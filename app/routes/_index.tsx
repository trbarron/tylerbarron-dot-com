...
function ProjectCard({ to, title, description }: ProjectLink) {
  return (
    <div className="border-4 border-black bg-white/90 backdrop-blur-sm transition-transform duration-150 ease-out motion-reduce:transition-none hover:-translate-x-[3px] hover:-translate-y-[3px] hover:shadow-[6px_6px_0_0_#111]">
                ...
                <ul className="bg-white/90 backdrop-blur-sm">
                  {items.map((post) => (
                    <li key={post.slug} className="border-b border-black/10 last:border-b-0">
                      <Link
                        to={`/blog/${post.slug}`}
                        className="grid grid-cols-[110px_1fr_24px] md:grid-cols-[140px_1fr_24px] gap-5 items-center px-3 py-3 no-underline hover:no-underline group transition-all duration-150 hover:bg-black hover:pl-6 motion-reduce:transition-none"
                      >
                        <span className="font-mono text-xs text-neutral-500 group-hover:text-white/80 uppercase">
                          {formatPostDate(post.date)}
                        </span>
                        <span className="font-neo font-black text-base md:text-lg text-black group-hover:text-white tracking-tight">
                          {post.title}
                        </span>
                        <span aria-hidden="true" className="text-right text-black/40 group-hover:text-white">→</span>
                      </Link>
                    </li>
                  ))}
                </ul>
