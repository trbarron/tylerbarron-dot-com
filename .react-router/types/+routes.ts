// Generated by React Router

import "react-router"

declare module "react-router" {
  interface Register {
    pages: Pages
    routeFiles: RouteFiles
    routeModules: RouteModules
  }
}

type Pages = {
  "/": {
    params: {};
  };
  "/blog/:slug": {
    params: {
      "slug": string;
    };
  };
  "/favicon.ico": {
    params: {};
  };
  "/healthcheck": {
    params: {};
  };
  "/blunder-watch": {
    params: {};
  };
  "/bouldering-tracker": {
    params: {};
  };
  "/camel-up-cup": {
    params: {};
  };
  "/camel-up-cup/2018": {
    params: {};
  };
  "/camel-up-cup/leaderboard": {
    params: {};
  };
  "/cat-tracker": {
    params: {};
  };
  "/cat-tracker/blog": {
    params: {};
  };
  "/chesser-guesser": {
    params: {};
  };
  "/collaborative-checkmate": {
    params: {};
  };
  "/collaborative-checkmate/:gameId/:playerId": {
    params: {
      "gameId": string;
      "playerId": string;
    };
  };
  "/generative-art": {
    params: {};
  };
  "/pizza-rating": {
    params: {};
  };
  "/the-riddler": {
    params: {};
  };
  "/boulderingTracker": {
    params: {};
  };
  "/BoulderingTracker": {
    params: {};
  };
  "/camelUpCup": {
    params: {};
  };
  "/CamelUpCup": {
    params: {};
  };
  "/catTracker": {
    params: {};
  };
  "/CatTracker": {
    params: {};
  };
  "/catTracker/blog": {
    params: {};
  };
  "/CatTracker/Blog": {
    params: {};
  };
  "/chesserGuesser": {
    params: {};
  };
  "/collaborativeCheckmate": {
    params: {};
  };
  "/collaborativeCheckmate/:gameId/:playerId": {
    params: {
      "gameId": string;
      "playerId": string;
    };
  };
  "/generativeArt": {
    params: {};
  };
  "/GenerativeArt": {
    params: {};
  };
  "/pizzaRating": {
    params: {};
  };
  "/PizzaRating": {
    params: {};
  };
  "/theRiddler": {
    params: {};
  };
  "/TheRiddler": {
    params: {};
  };
  "/set": {
    params: {};
  };
  "/SSBM": {
    params: {};
  };
  "/robots.txt": {
    params: {};
  };
  "/sitemap.xml": {
    params: {};
  };
  "/feed.xml": {
    params: {};
  };
  "/giscus-theme.css": {
    params: {};
  };
  "/api/chesserGuesser/puzzles": {
    params: {};
  };
  "/api/chesserGuesser/submit": {
    params: {};
  };
  "/api/chesserGuesser/leaderboard": {
    params: {};
  };
  "/api/camelUpCup/submit": {
    params: {};
  };
  "/api/camelUpCup/status": {
    params: {};
  };
  "/api/blunderWatch/game": {
    params: {};
  };
  "/api/blunderWatch/submit": {
    params: {};
  };
  "/api/blunderWatch/leaderboard": {
    params: {};
  };
  "/multiple-choice-chess": {
    params: {};
  };
  "/multiple-choice-chess/:gameId/:playerId": {
    params: {
      "gameId": string;
      "playerId": string;
    };
  };
  "/api/multipleChoiceChess/create": {
    params: {};
  };
  "/api/multipleChoiceChess/join": {
    params: {};
  };
  "/api/multipleChoiceChess/state": {
    params: {};
  };
  "/api/multipleChoiceChess/move": {
    params: {};
  };
  "/api/multipleChoiceChess/resign": {
    params: {};
  };
  "/api/multipleChoiceChess/claimWin": {
    params: {};
  };
  "/api/multipleChoiceChess/available": {
    params: {};
  };
  "/*": {
    params: {
      "*": string;
    };
  };
};

type RouteFiles = {
  "root.tsx": {
    id: "root";
    page: "/" | "/blog/:slug" | "/favicon.ico" | "/healthcheck" | "/blunder-watch" | "/bouldering-tracker" | "/camel-up-cup" | "/camel-up-cup/2018" | "/camel-up-cup/leaderboard" | "/cat-tracker" | "/cat-tracker/blog" | "/chesser-guesser" | "/collaborative-checkmate" | "/collaborative-checkmate/:gameId/:playerId" | "/generative-art" | "/pizza-rating" | "/the-riddler" | "/boulderingTracker" | "/BoulderingTracker" | "/camelUpCup" | "/CamelUpCup" | "/catTracker" | "/CatTracker" | "/catTracker/blog" | "/CatTracker/Blog" | "/chesserGuesser" | "/collaborativeCheckmate" | "/collaborativeCheckmate/:gameId/:playerId" | "/generativeArt" | "/GenerativeArt" | "/pizzaRating" | "/PizzaRating" | "/theRiddler" | "/TheRiddler" | "/set" | "/SSBM" | "/robots.txt" | "/sitemap.xml" | "/feed.xml" | "/giscus-theme.css" | "/api/chesserGuesser/puzzles" | "/api/chesserGuesser/submit" | "/api/chesserGuesser/leaderboard" | "/api/camelUpCup/submit" | "/api/camelUpCup/status" | "/api/blunderWatch/game" | "/api/blunderWatch/submit" | "/api/blunderWatch/leaderboard" | "/multiple-choice-chess" | "/multiple-choice-chess/:gameId/:playerId" | "/api/multipleChoiceChess/create" | "/api/multipleChoiceChess/join" | "/api/multipleChoiceChess/state" | "/api/multipleChoiceChess/move" | "/api/multipleChoiceChess/resign" | "/api/multipleChoiceChess/claimWin" | "/api/multipleChoiceChess/available" | "/*";
  };
  "routes/_index.tsx": {
    id: "routes/_index";
    page: "/";
  };
  "routes/blog.$slug.tsx": {
    id: "routes/blog.$slug";
    page: "/blog/:slug";
  };
  "routes/favicon[.]ico.ts": {
    id: "routes/favicon[.]ico";
    page: "/favicon.ico";
  };
  "routes/healthcheck.tsx": {
    id: "routes/healthcheck";
    page: "/healthcheck";
  };
  "routes/blunderWatch.tsx": {
    id: "routes/blunderWatch";
    page: "/blunder-watch";
  };
  "routes/boulderingTracker.tsx": {
    id: "routes/boulderingTracker";
    page: "/bouldering-tracker";
  };
  "routes/camelUpCup._index.tsx": {
    id: "routes/camelUpCup._index";
    page: "/camel-up-cup";
  };
  "routes/camelUpCup.2018.tsx": {
    id: "routes/camelUpCup.2018";
    page: "/camel-up-cup/2018";
  };
  "routes/camelUpCup.leaderboardRedirect.ts": {
    id: "routes/camelUpCup.leaderboardRedirect";
    page: "/camel-up-cup/leaderboard";
  };
  "routes/catTracker._index.tsx": {
    id: "routes/catTracker._index";
    page: "/cat-tracker";
  };
  "routes/catTracker.blog.tsx": {
    id: "routes/catTracker.blog";
    page: "/cat-tracker/blog";
  };
  "routes/chesserGuesser.tsx": {
    id: "routes/chesserGuesser";
    page: "/chesser-guesser";
  };
  "routes/collaborativeCheckmate._index.tsx": {
    id: "routes/collaborativeCheckmate._index";
    page: "/collaborative-checkmate";
  };
  "routes/collaborativeCheckmate.$gameId.$playerId.tsx": {
    id: "routes/collaborativeCheckmate.$gameId.$playerId";
    page: "/collaborative-checkmate/:gameId/:playerId";
  };
  "routes/generativeArt.tsx": {
    id: "routes/generativeArt";
    page: "/generative-art";
  };
  "routes/pizzaRating.tsx": {
    id: "routes/pizzaRating";
    page: "/pizza-rating";
  };
  "routes/theRiddler.tsx": {
    id: "routes/theRiddler";
    page: "/the-riddler";
  };
  "routes/legacyRedirect.ts": {
    id: "legacy-bouldering-tracker";
    page: "/boulderingTracker";
  } | {
    id: "legacy-bouldering-tracker-pascal";
    page: "/BoulderingTracker";
  } | {
    id: "legacy-camel-up-cup";
    page: "/camelUpCup";
  } | {
    id: "legacy-camel-up-cup-pascal";
    page: "/CamelUpCup";
  } | {
    id: "legacy-cat-tracker";
    page: "/catTracker";
  } | {
    id: "legacy-cat-tracker-pascal";
    page: "/CatTracker";
  } | {
    id: "legacy-cat-tracker-blog";
    page: "/catTracker/blog";
  } | {
    id: "legacy-cat-tracker-blog-pascal";
    page: "/CatTracker/Blog";
  } | {
    id: "legacy-chesser-guesser";
    page: "/chesserGuesser";
  } | {
    id: "legacy-collaborative-checkmate";
    page: "/collaborativeCheckmate";
  } | {
    id: "legacy-collaborative-checkmate-game";
    page: "/collaborativeCheckmate/:gameId/:playerId";
  } | {
    id: "legacy-generative-art";
    page: "/generativeArt";
  } | {
    id: "legacy-generative-art-pascal";
    page: "/GenerativeArt";
  } | {
    id: "legacy-pizza-rating";
    page: "/pizzaRating";
  } | {
    id: "legacy-pizza-rating-pascal";
    page: "/PizzaRating";
  } | {
    id: "legacy-the-riddler";
    page: "/theRiddler";
  } | {
    id: "legacy-the-riddler-pascal";
    page: "/TheRiddler";
  };
  "routes/set.tsx": {
    id: "routes/set";
    page: "/set";
  };
  "routes/SSBM.tsx": {
    id: "routes/SSBM";
    page: "/SSBM";
  };
  "routes/robots[.]txt.ts": {
    id: "routes/robots[.]txt";
    page: "/robots.txt";
  };
  "routes/sitemap[.]xml.ts": {
    id: "routes/sitemap[.]xml";
    page: "/sitemap.xml";
  };
  "routes/feed[.]xml.ts": {
    id: "routes/feed[.]xml";
    page: "/feed.xml";
  };
  "routes/giscus-theme[.]css.ts": {
    id: "routes/giscus-theme[.]css";
    page: "/giscus-theme.css";
  };
  "routes/api/chesserGuesser/puzzles.ts": {
    id: "routes/api/chesserGuesser/puzzles";
    page: "/api/chesserGuesser/puzzles";
  };
  "routes/api/chesserGuesser/submit.ts": {
    id: "routes/api/chesserGuesser/submit";
    page: "/api/chesserGuesser/submit";
  };
  "routes/api/chesserGuesser/leaderboard.ts": {
    id: "routes/api/chesserGuesser/leaderboard";
    page: "/api/chesserGuesser/leaderboard";
  };
  "routes/api/camelUpCup/submit.ts": {
    id: "routes/api/camelUpCup/submit";
    page: "/api/camelUpCup/submit";
  };
  "routes/api/camelUpCup/status.ts": {
    id: "routes/api/camelUpCup/status";
    page: "/api/camelUpCup/status";
  };
  "routes/api/blunderWatch/game.ts": {
    id: "routes/api/blunderWatch/game";
    page: "/api/blunderWatch/game";
  };
  "routes/api/blunderWatch/submit.ts": {
    id: "routes/api/blunderWatch/submit";
    page: "/api/blunderWatch/submit";
  };
  "routes/api/blunderWatch/leaderboard.ts": {
    id: "routes/api/blunderWatch/leaderboard";
    page: "/api/blunderWatch/leaderboard";
  };
  "routes/multipleChoiceChess._index.tsx": {
    id: "routes/multipleChoiceChess._index";
    page: "/multiple-choice-chess";
  };
  "routes/multipleChoiceChess.$gameId.$playerId.tsx": {
    id: "routes/multipleChoiceChess.$gameId.$playerId";
    page: "/multiple-choice-chess/:gameId/:playerId";
  };
  "routes/api/multipleChoiceChess/create.ts": {
    id: "routes/api/multipleChoiceChess/create";
    page: "/api/multipleChoiceChess/create";
  };
  "routes/api/multipleChoiceChess/join.ts": {
    id: "routes/api/multipleChoiceChess/join";
    page: "/api/multipleChoiceChess/join";
  };
  "routes/api/multipleChoiceChess/state.ts": {
    id: "routes/api/multipleChoiceChess/state";
    page: "/api/multipleChoiceChess/state";
  };
  "routes/api/multipleChoiceChess/move.ts": {
    id: "routes/api/multipleChoiceChess/move";
    page: "/api/multipleChoiceChess/move";
  };
  "routes/api/multipleChoiceChess/resign.ts": {
    id: "routes/api/multipleChoiceChess/resign";
    page: "/api/multipleChoiceChess/resign";
  };
  "routes/api/multipleChoiceChess/claimWin.ts": {
    id: "routes/api/multipleChoiceChess/claimWin";
    page: "/api/multipleChoiceChess/claimWin";
  };
  "routes/api/multipleChoiceChess/available.ts": {
    id: "routes/api/multipleChoiceChess/available";
    page: "/api/multipleChoiceChess/available";
  };
  "routes/$.tsx": {
    id: "routes/$";
    page: "/*";
  };
};

type RouteModules = {
  "root": typeof import("./app/root.tsx");
  "routes/_index": typeof import("./app/routes/_index.tsx");
  "routes/blog.$slug": typeof import("./app/routes/blog.$slug.tsx");
  "routes/favicon[.]ico": typeof import("./app/routes/favicon[.]ico.ts");
  "routes/healthcheck": typeof import("./app/routes/healthcheck.tsx");
  "routes/blunderWatch": typeof import("./app/routes/blunderWatch.tsx");
  "routes/boulderingTracker": typeof import("./app/routes/boulderingTracker.tsx");
  "routes/camelUpCup._index": typeof import("./app/routes/camelUpCup._index.tsx");
  "routes/camelUpCup.2018": typeof import("./app/routes/camelUpCup.2018.tsx");
  "routes/camelUpCup.leaderboardRedirect": typeof import("./app/routes/camelUpCup.leaderboardRedirect.ts");
  "routes/catTracker._index": typeof import("./app/routes/catTracker._index.tsx");
  "routes/catTracker.blog": typeof import("./app/routes/catTracker.blog.tsx");
  "routes/chesserGuesser": typeof import("./app/routes/chesserGuesser.tsx");
  "routes/collaborativeCheckmate._index": typeof import("./app/routes/collaborativeCheckmate._index.tsx");
  "routes/collaborativeCheckmate.$gameId.$playerId": typeof import("./app/routes/collaborativeCheckmate.$gameId.$playerId.tsx");
  "routes/generativeArt": typeof import("./app/routes/generativeArt.tsx");
  "routes/pizzaRating": typeof import("./app/routes/pizzaRating.tsx");
  "routes/theRiddler": typeof import("./app/routes/theRiddler.tsx");
  "legacy-bouldering-tracker": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-bouldering-tracker-pascal": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-camel-up-cup": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-camel-up-cup-pascal": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-cat-tracker": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-cat-tracker-pascal": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-cat-tracker-blog": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-cat-tracker-blog-pascal": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-chesser-guesser": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-collaborative-checkmate": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-collaborative-checkmate-game": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-generative-art": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-generative-art-pascal": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-pizza-rating": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-pizza-rating-pascal": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-the-riddler": typeof import("./app/routes/legacyRedirect.ts");
  "legacy-the-riddler-pascal": typeof import("./app/routes/legacyRedirect.ts");
  "routes/set": typeof import("./app/routes/set.tsx");
  "routes/SSBM": typeof import("./app/routes/SSBM.tsx");
  "routes/robots[.]txt": typeof import("./app/routes/robots[.]txt.ts");
  "routes/sitemap[.]xml": typeof import("./app/routes/sitemap[.]xml.ts");
  "routes/feed[.]xml": typeof import("./app/routes/feed[.]xml.ts");
  "routes/giscus-theme[.]css": typeof import("./app/routes/giscus-theme[.]css.ts");
  "routes/api/chesserGuesser/puzzles": typeof import("./app/routes/api/chesserGuesser/puzzles.ts");
  "routes/api/chesserGuesser/submit": typeof import("./app/routes/api/chesserGuesser/submit.ts");
  "routes/api/chesserGuesser/leaderboard": typeof import("./app/routes/api/chesserGuesser/leaderboard.ts");
  "routes/api/camelUpCup/submit": typeof import("./app/routes/api/camelUpCup/submit.ts");
  "routes/api/camelUpCup/status": typeof import("./app/routes/api/camelUpCup/status.ts");
  "routes/api/blunderWatch/game": typeof import("./app/routes/api/blunderWatch/game.ts");
  "routes/api/blunderWatch/submit": typeof import("./app/routes/api/blunderWatch/submit.ts");
  "routes/api/blunderWatch/leaderboard": typeof import("./app/routes/api/blunderWatch/leaderboard.ts");
  "routes/multipleChoiceChess._index": typeof import("./app/routes/multipleChoiceChess._index.tsx");
  "routes/multipleChoiceChess.$gameId.$playerId": typeof import("./app/routes/multipleChoiceChess.$gameId.$playerId.tsx");
  "routes/api/multipleChoiceChess/create": typeof import("./app/routes/api/multipleChoiceChess/create.ts");
  "routes/api/multipleChoiceChess/join": typeof import("./app/routes/api/multipleChoiceChess/join.ts");
  "routes/api/multipleChoiceChess/state": typeof import("./app/routes/api/multipleChoiceChess/state.ts");
  "routes/api/multipleChoiceChess/move": typeof import("./app/routes/api/multipleChoiceChess/move.ts");
  "routes/api/multipleChoiceChess/resign": typeof import("./app/routes/api/multipleChoiceChess/resign.ts");
  "routes/api/multipleChoiceChess/claimWin": typeof import("./app/routes/api/multipleChoiceChess/claimWin.ts");
  "routes/api/multipleChoiceChess/available": typeof import("./app/routes/api/multipleChoiceChess/available.ts");
  "routes/$": typeof import("./app/routes/$.tsx");
};